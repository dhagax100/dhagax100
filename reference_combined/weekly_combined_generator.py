...
def write_combined_pine(base: Path, engine: WeeklyCombinedEngine, label_cap: int, ob_cap: int, rb_cap: int,
    ...
    # ---- global rank: 1 = the single most recent zone of ANY type (by
    # origin week), 2 = the one before that, and so on -- lets "POI from
    # last" mean the same thing under Focus POI = ALL that it means for a
    # single type: a chronological "how far back" count, just across all
    # three types at once instead of within one. Per-type rank (below,
    # unchanged) still applies whenever a specific type is focused.
    global_entries = []
    for z in engine.ob_zones:
        if not z.rejected:
            global_entries.append((engine.w[z.candle].start, "OB", z.id))
    for z in engine.rb_zones:
        global_entries.append((engine.w[z.candle].start, "RB", z.id))
    for z in engine.fvg_zones:
        global_entries.append((engine.w[z.left].start, "FVG", z.id))
    global_entries.sort(key=lambda e: e[0], reverse=True)
    global_rank_map = {(poi, zid): idx + 1 for idx, (_, poi, zid) in enumerate(global_entries)}
    max_global_rank = max(1, len(global_entries))
    ...
    lines += [
        ...
        *pack_array("tRank", "int", t_rank), *pack_array("tGRank", "int", t_grank),
        ...
        "                if not inspectOnePoi or (focusPoi == \"ALL\" ? array.get(obGRank, i) == poiFromLast : array.get(obRank, i) == poiFromLast)",
        ...
        "                if not inspectOnePoi or (focusPoi == \"ALL\" ? array.get(rbGRank, i) == poiFromLast : array.get(rbRank, i) == poiFromLast)",
        ...
        "                if not inspectOnePoi or (focusPoi == \"ALL\" ? array.get(fvgGRank, i) == poiFromLast : array.get(fvgRank, i) == poiFromLast)",
