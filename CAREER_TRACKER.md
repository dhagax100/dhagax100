# Career & Opportunity Tracker

_Last updated: 2026-08-09_
_This file is the single source of truth for active opportunities, deadlines, and decisions. Updated every time something changes; reviewed automatically every week._

---

## 1. Financial Baseline (KFUPM PhD offer, starting Jan 2027)

Use this to judge whether any other opportunity is actually worth it.

| Component | Est. annual value | Notes |
|---|---|---|
| Cash stipend | $16,000 | $1,333/month |
| Housing (waived) | ~$5,400 | assumption, unconfirmed |
| Utilities: electricity/water/internet (waived) | ~$1,600 | assumption, unconfirmed |
| Transportation (free) | ~$800 | assumption |
| Sports facility access | ~$300 | assumption |
| **Living-standard baseline (cash + waived essentials)** | **~$24,000–24,500/yr (~$2,000–2,050/mo)** | **Use this to test any job's take-home pay** |
| Tuition (waived) | ~$18,000–25,000 | rough estimate, unconfirmed |
| Publication fees (paid by university) | ~$1,500 | variable |
| **Full package value (incl. tuition/pubs)** | **~$44,000–46,500/yr** | long-term/opportunity-cost framing, not spendable cash |

**Rule of thumb going forward:** any external offer's take-home pay (after that job's own rent/utilities/transport) needs to clear ~$2,000/month, adjusted for local cost of living, to match staying on the PhD track day-to-day. Below that, the PhD track wins on pure economics — anything above it needs a non-financial reason to justify a switch or delay.

---

## 2. Active Opportunities

| Opportunity | Status | Key dates | Next action |
|---|---|---|---|
| **KFUPM PhD** (Mechanical Engineering) | Admitted, scholarship confirmed | MSc graduation: Dec 2026. PhD starts: Jan 2027 | None — on track |
| **International exit search** (Ireland, Canada, Australia, NZ, Germany, Iceland, UK, Qatar) | Research complete — see full dashboard | Target start: **Jan 2027**, right after leaving KFUPM | User to pick 1–3 countries to pursue; then draft tailored CV/cover letter per real vacancy |

~~Hormuud Telecom — Data Center Operations Engineer (Mogadishu)~~ — **dropped** per user decision (2026-08-09), not pursuing further.

## 3. International Opportunity Search (started 2026-08-09)

Full comparison dashboard (money, tax, citizenship speed, weather, visa ease, spouse work rights, per-country job market signal, sources): **[Exit Plan — Opportunity Map](https://claude.ai/code/artifact/9e961089-9361-479d-968e-d7af7c55f3c1)**

**Search criteria (user-set):** money first; nationality-granting countries preferred; low tax preferred; good weather preferred; fastest path to citizenship preferred; easy/clear visa process preferred; industry roles over academic/research; must check if spouse (no degree/skill) gets a visa + work rights; not limited to the 6 named countries.

**Quick summary — take-home/month estimate vs. PhD baseline ($2,000–2,050/mo):**

| Country | Est. take-home/mo (USD) | Citizenship | Spouse can work | Verdict |
|---|---|---|---|---|
| Ireland (DC specialist, 2-3yrs in) | ~$4,500–5,000 | 5 yrs (full labor market at 21mo) | Yes, full | **Top pick — best industry fit** |
| New Zealand | ~$2,900–3,600 | 5 yrs, PR instant | Yes, open rights | **Top pick — easiest process** |
| Canada | ~$3,000–3,300 | 4–5 yrs | Yes (2026 rules tightened) | Strong pick |
| Iceland | ~$4,000–4,400 | 7–9 yrs (slowest) | Yes, automatic | High pay, slow citizenship |
| Australia | ~$3,000–3,200 | 4 yrs | Yes, full | Solid all-around |
| Germany | ~$2,500–2,700 | PR in 21-27mo, citizenship 5yrs | Yes, full | Fastest PR, high tax |
| UK | ~$3,400–3,600 (if sponsored) | 5yrs now, may become 10 (Autumn 2026 risk) | Yes, full | Fresh-grad sponsorship barrier — watch, don't lead with |
| Qatar | ~$1,900–3,850 | **Never** | Only with her own permit | Good money, fails nationality condition |

### 3a. Key Dates — confirmed by user

| Opportunity | Application opens / closes | Interview | Notes |
|---|---|---|---|
| **⭐ TOP PRIORITY — University of Calgary — Fully Funded MSc/PhD, Hypersonic Modeling** (Calgary, Canada) | Rolling (no fixed closure) — planned start Sept 2026, **but explicitly stated as negotiable** | Online interview for shortlisted candidates only | User's #1 priority. **Negotiable start date solves the Dec-2026-graduation conflict that every other listing has.** Computational Fluid & Structural Mechanics Group; hypersonic vehicles, thermo-chemical non-equilibrium, ML for reduced-order modeling. Contact: Prof. Artem Korobenko, artem.korobenko@ucalgary.ca (put project name in subject line). **Apply by emailing:** CV + unofficial transcripts + English test results + 1-page cover letter. Academic CV + tailored cover letter prepared and sent. **User still needs to personally attach:** unofficial transcripts, and English test results — his Duolingo score (130/160) may or may not satisfy "English test results," worth confirming with Prof. Korobenko directly since IELTS/TOEFL is more commonly expected. Honest gap flagged in the letter: no hypersonic-specific FEM/FVM flow-solver experience — background is energy-systems CFD, not aerospace. |
| **🔴 LiU — PhD Student, AI-Based Flow Control for Hydropower** (Linköping, Sweden) | **Closes 31 Aug 2026** | TBD | URGENT — 22 days from discovery. Fully funded. Contacts: Saeed Salehi (main supervisor, saeed.salehi@liu.se), Roland Gårdhagen (head of division, roland.gardhagen@liu.se). Apply: https://liu.se/en/work-at-liu/vacancies/29533. Academic CV + motivation letter prepared and sent. |
| **🔴 Western University — PhD, CFD for Advanced Oxidation** (London, Ontario, Canada) | **Rolling basis, posted closure 1 Sept 2026** — reviewed as applications arrive, so early submission matters | TBD, only shortlisted candidates contacted | URGENT. Fully funded. Apply by **emailing CV directly** to Dr. Domenico Santoro, Adjunct Research Professor, Western University — santoro.office@gmail.com — mention "CFD Jobs Database record #20098". Academic CV + application email prepared. |
| **🔴 Paul Wurth — CFD-DEM Modelling of Raceway for Low-CO2 Blast Furnace** (Luxembourg) | **Closes 31 Aug 2026** | TBD | URGENT. Genuinely industry, not pure academic — employed by Paul Wurth under an FNR AFR PhD grant with an employment contract, funded up to 4 years. Matches user's stated preference for industry over academic. Apply by **emailing CV** to Dr. Mehdi Baniasadi — baniasadi1984@gmail.com — mention "CFD Jobs Database record #20071". Gap to flag honestly: no DEM software experience (EDEM/LIGGGHTS) and not an EU resident (listed as "an asset," not a requirement). Academic CV reused; application email drafted. |
| **🟡 University of Manchester — PhD, Multi-Physics Modelling of Particle-Based Energy Storage Systems** (Manchester, UK) | **Application closes 1 Oct 2026** (this is the deadline, not confirmed start date — user assumed Oct 2026 start, needs checking) | TBD | User wants to apply regardless of timing. Strong fit: thermal energy storage + CFD + renewable energy under variable wind/solar conditions — matches HOMER/SAM and thermal-CFD (parabolic trough) background closely. **Incomplete info** — screenshot cut off before qualifications/contact details; waiting on user for the rest of the listing before an application can be drafted. |
| **🔴 Loughborough University — PhD, CFD and Data-Driven Fluid Mechanics (Sanctuary Scholarship)** (Leicestershire, UK) | Rolling (no fixed closure). Entry points: Oct 2026 (not viable — user still enrolled), **Feb 2027 (realistic target)**, or Jul 2027 | TBD | URGENT per user, but **eligibility unconfirmed** — Sanctuary Scholarships typically require refugee/asylum/humanitarian-protection status, not just nationality from a conflict-affected country. User believes he may qualify as Somali with "a true story to tell" but has not yet shared specifics, and his actual immigration situation (KFUPM-sponsored student in Saudi Arabia) may not fit the usual criteria. **Not proceeding with a personal narrative until user confirms his actual story and/or the eligibility is confirmed directly with the contact.** Covers 100% tuition + £21,805/yr stipend (UKRI rate), 3 years — but user would need to self-fund travel/visa costs. Research fit: DNS of non-ideal compressible fluids, data-driven fluid mechanics, turbomachinery aerodynamics — reasonable CFD fit via his ANSYS Fluent background. Contact: Bijie Yang, b.yang@lboro.ac.uk, mention "CFD Jobs Database record #20066". |
| IDCORE EngD (Edinburgh, Offshore Renewable Energy) | Applications open **30 Nov 2026** | **Jan 2027** (user wrote "Jan 2026" — almost certainly a typo, since that's before the application even opens; treating as Jan 2027 until confirmed) | SOP already prepared and sent |
| HBKU — PhD in Sustainable Energy (Qatar) | **Nov 2026 – Feb 2027** window | TBD | Requires IELTS + GRE — user does not have these yet, needs to prepare soon |

**Action needed now (Aug–Nov 2026):**
1. Submit LiU and Western University applications ASAP — both are live and time-sensitive right now.
2. IELTS and GRE both take real lead time — registration, prep, results turnaround. With the HBKU window opening in ~3 months, booking test dates should start soon, not in November.

**Source note:** both LiU and Western University roles were found by the user directly on cfd-online.com/Jobs — worth the user checking that board personally on a regular basis; it surfaces roles I can't browse into myself in this sandbox.

## 4. Known Conflict / Open Question

- ~~Hormuud role would collide with PhD start date~~ — resolved: Hormuud dropped.
- **Open:** which 1–3 countries to actively pursue? Waiting on user decision.

---

## 5. Decisions Log

| Date | Decision |
|---|---|
| 2026-08 | Tailored CV and cover letter completed for Hormuud application, using DEMA data-center workshop + ISO 50001 training to offset lack of direct data-center work experience |
| 2026-08 | Established $2,000/month living-standard baseline to evaluate future opportunities against |
| 2026-08-09 | Dropped Hormuud from active pursuit; launched full international opportunity search (money-first, nationality-preferred, industry roles) across 8 countries, not limited to original 6 |

---

## 6. Application Files Prepared (2026-08-09)

Not narrowing to countries — pursuing every real, apply-ready match at once, per user decision. Graduation confirmed: December 2026.

| Target | Docs prepared | Status |
|---|---|---|
| Equinix — Slough, UK (Electrical & Mechanical Facilities Engineer) | Master CV + tailored cover letter | Ready to submit via careers.equinix.com |
| Equinix — Manchester, UK (same role type) | Master CV + tailored cover letter | Ready to submit via careers.equinix.com |
| Verne Global — Keflavík, Iceland | Master CV + tailored cover letter (generic — exact open role title unconfirmed, user must check careers.letsverne.com/jobs) | Ready, pending role confirmation |
| NTT Global Data Centers — DACH region (Frankfurt/Munich/Zurich) | Master CV + tailored cover letter (raises the German-language question directly) | Ready to submit |
| IDCORE EngD, Offshore Renewable Energy — Edinburgh | Statement of Purpose | Ready — apply via Edinburgh degree finder; note: UK doctorates start Sept, not Jan |
| AirTrunk — Sydney/Melbourne | **Dropped** — role requires 8 yrs experience (5 in data centres), a hard structural mismatch, not a fit role | Not pursuing; keep as a future target company only |

**Correction logged:** user does not want "avoid academic," only "prefer industry over academic" — IDCORE stays fully in play. Also corrected: don't manufacture soft gaps (e.g. UK driving licence) as disqualifiers in cover letters — only real, hard-blocking requirements (like AirTrunk's 8-year minimum) should exclude a role.

## 7. Next Actions

- [ ] **⭐ PRIORITY — Submit the University of Calgary hypersonic PhD application** (CV + cover letter + transcripts + English test) by email to artem.korobenko@ucalgary.ca — documents ready and sent to user on 2026-08-10, not yet confirmed submitted. Remind user every check-in until confirmed sent.
- [ ] User to actually submit the 5 prepared industry applications (Equinix x2, Verne, NTT, IDCORE)
- [ ] Confirm exact open role title at Verne Global before submitting
- [ ] Continue finding more real, apply-ready roles (Canada, Australia, Netherlands, Italy, Scotland/Wales graduate schemes)
- [ ] Broaden the Mon/Thu Routine to actively rescan for new postings, not just review this file
- [ ] Get real KFUPM figures (housing cost, tuition sticker price) to tighten the financial baseline estimate
- [ ] **Confirm IDCORE interview month** — user wrote "Jan 2026," likely means Jan 2027
- [ ] Book IELTS test date (needed for HBKU Sustainable Energy PhD application, window opens Nov 2026)
- [ ] Book GRE test date (same requirement, same deadline pressure)
- [ ] Prepare HBKU Sustainable Energy PhD application materials once test scores are in hand

---

_This file is checked automatically on a weekly Routine. Update it any time something changes so the check-in stays accurate._
