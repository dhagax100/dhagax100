...
// Weekly IMPACTED order blocks, pre-computed from FXCM 1-minute {feed} data.
// Each box spans OB candle -> exact impact minute; the vertical line marks the
// exact impact minute. Load next to your live indicator to compare.
indicator("Pre-calc Weekly OB (impacted)", overlay=true, max_boxes_count=500, max_lines_count=500, max_labels_count=500)

// per OB: "candleMs:zb:zt:bull:impactMs"
var string WOB = "{wob}"
...
if barstate.islast and str.length(WOB) > 0
    ...
    for ti = 0 to array.size(toks) - 1
        ...
        if array.size(f) == 5
            ...
            bool bull = array.get(f, 3) == "1"
            int it = int(str.tonumber(array.get(f, 4)))
            color c = bull ? color.blue : color.black
            box.new(ct, zt, it, zb, xloc=xloc.bar_time, border_color=color.new(c,0), border_width=2, bgcolor=na)
...
def main():
    ...
    impacted = [o for o in obs if o.state == 3 and o.impactTime >= 0 and not o.rejected]
    print(f"\nTotal OBs: {len(obs)}.  Impacted: {len(impacted)}.\n")

    print("IMPACTED WEEKLY OBs (side | type | trigger | eligibility | impact | zone):")
    for i, o in enumerate(sorted(impacted, key=lambda z: z.impactTime), 1):
        side = "BUY " if o.bullish else "SELL"
        print(f"  #{i:>2} {side} {_STATE[o.origState]:<5} "
              f"trig {_fmt(trigger_time(o))}  elig {_fmt(o.eligibleTime)}  "
              f"impact {_fmt(o.impactTime)}  [{o.zb:.5f}-{o.zt:.5f}]")

    wob = ",".join(f"{BTms[o.candle]}:{o.zb:.5f}:{o.zt:.5f}:{1 if o.bullish else 0}:{o.impactTime}"
                   for o in impacted)
