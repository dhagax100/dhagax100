...
- User then asked directly whether selling actually resumes somewhere inside that NONE window, before zone #5 gets touched. Checked `h4_ob_ledger.csv` for any H4-level impact (either side) between 2026-05-29 17:51:00 and 2026-06-05 18:37:00: found **H4 OB #206 (SELL, AOB), impacted 2026-06-05 05:03:00 Riyadh** -- roughly 13.5 hours before zone #5's own touch. That is a genuine sell-side resumption attempt inside the NONE window; nothing else (either side) shows up anywhere earlier in the gap.
- #206 is already marked `authorized=False` in the ledger, but for the wrong reason: the code believes control is already BUY_ONLY there (it snaps `control_at_impact`/`parent_weekly_id` to zone 5/BUY_ONLY at the week-22 boundary, 2026-06-01, days before zone 5 is actually touched -- a second, related symptom of the same underlying gap: control transitions are being assigned at whole-week granularity, not at the exact minute the triggering event happens). The correct reason it should be unauthorized is SS16's NONE state, not an early BUY_ONLY snap. Same root cause as above, not a separate bug -- folded into this same test case rather than opening a new entry.
