...
**CORRECTED final chain** (this supersedes the looser prose version above -- the full corrected sequence, now also encoded into `build_manual_gates()` in `full_viewer.py`, all 14 gates, zone 3's original impact through the end of the dataset):

| # | Control | From | Trigger |
|---|---|---|---|
| 1 | SELL_ONLY (zone 3) | 04-14 17:55 | zone 3 impacted |
| 2 | NONE | 05-29 17:51 | swing low confirmed |
| 3 | SELL_ONLY (zone 3) | 06-05 16:51 | that swing low taken out (old rule, kept as-is) |
| 4 | BOTH (3/5) | 06-05 18:37 | opposing zone 5 impacted |
| 5 | SELL_ONLY (zone 3) | 06-08 00:00 | buy side (zone 5) exhausted |
| 6 | NONE | 06-15 00:29 | swing low confirmed |
| 7 | SELL_ONLY (zone 3) | 06-17 22:24 | that swing low taken out (old rule; new rule gives the identical minute here) |
| 8 | NONE | 07-14 15:30 | swing low confirmed |
| 9 | SELL_ONLY (zone 3) | 07-23 15:43 | swing HIGH confirmed (new rule) |
| 10 | NONE | 07-29 21:53 | new swing low confirmed, no buy in control |
| 11 | SELL_ONLY (**zone 9**) | 07-30 16:48 | first zone reaction from a plain NONE -- **exactly one entry**, then zone 9 dies |
| 12 | NONE | **08-03 00:00** | zone 9's own containing week (07-27 to 08-03) closes at 1.15441, INSIDE its box (zb 1.15072 / zt 1.15693), no clean wick-reject -- dies. Nothing tradable until zone 8 reacts. |
| 13 | SELL_ONLY (**zone 8**) | 08-19 15:49 | zone 8 reacts -- **exactly one entry**, then zone 8 dies |
| 14 | NONE | **08-24 00:00** | zone 8's own containing week (08-17 to 08-24, the SAME week it got impacted in) closes at 1.16718, a FULL BODY BREACH above its own zt (1.16522) -- dies outright. No tradable SELL zone left anywhere. Stays NONE through the last 1-minute bar in the CSV, **2026-09-11 22:05** -- recorded as the final state, nothing further exists to check. |

Gates 11 and 13 are each a single, standalone entry off a distinct Weekly zone in the same SELL lineage (zone 9, then zone 8) -- not zone 3's own original box, which has been dead/irrelevant since long before this. Verified: `h4_ob_ledger.csv` shows `control_at_impact = NONE` / `authorized = False` for every H4 OB impacted inside the 08-03 -> 08-19 idle gap (17 such rows checked), confirming the block works as intended.

**Encoded into code**: `build_manual_gates()` in `full_viewer.py` now returns all 14 gates above (was 8, stopping at 08-04 NONE). Regenerated and verified: 1130 4H bars, 359 H4 OBs computed, 26 shown in the `--manual-gates` window (up from 21), 31 5m BSO rows (up from 25).
...
## Session update — 2026-09-18 (control-gate port into weekly_control_engine.py COMPLETE; 2 new gates found and confirmed real)

**The port flagged as the next concrete task above is done.** `weekly_control_engine.py` now independently reproduces the full control-gate sequence from raw data -- the body-close-inside/through zone-death rule, the swing-high-confirms resume rule, and the first-zone-reaction-from-a-plain-NONE rule are all implemented in the engine itself, not just hand-maintained in `build_manual_gates()`.

**Real bugs found and fixed while validating the port against the hand-verified chain (three total this pass, on top of the three from the prior WIP commits):**

1. **Stale-zone bug (the big one).** The SWING_PAUSE check ("no opposing zone in control -> NONE") originally asked whether any opposing-direction zone had ever been `is_spent_state` (impacted, not rejected) -- but that stays true FOREVER once a zone is ever impacted, even long after that zone's own control episode already ended via ZONE_DEATH. Zone #1's BUY campaign (spent + died back at week 10, months before zone #3's SELL_ONLY campaign even starts at week 15) kept being counted as "an opposing zone in control" for the rest of the dataset, silently blocking every later SWING_PAUSE check. Root cause: the check was redundant in the first place -- `control` being single-direction (SELL_ONLY/BUY_ONLY, as opposed to BOTH) already IS the fact that no concurrent opposition exists, by construction of the existing OPPOSING_ENCOUNTER check (which promotes to BOTH the instant an opposing zone is impacted). Fixed by dropping the re-check entirely for the plain single-direction case, and adding the previously-missing BOTH-side case (a same-direction pause while BOTH hands full control to the surviving spent zone on the other side, per the user's stated "if we come to both... buying keeps the control" exception).

2. **Zone-death same-week-as-impact bug.** ZONE_DEATH was only ever checked against whoever was controlling BEFORE the current week started (a candidate built once at the top of the loop). A zone that gets impacted AND dies to its own Weekly-close body rule in the SAME week (zone 9: impacted 07-30, its own containing week closes 08-03 -- exactly the case gates 13->14 already describe) could never be detected that way, since `controlling_zone_id` only becomes that zone partway through the week's own trigger processing. This silently pushed zone 9's and zone 8's deaths one full week late. Fixed by moving the zone-death check to run AFTER the whole in-week trigger walk, against whoever ends up controlling by that point, rather than pre-built off the start-of-week state.

3. **Real, previously-missed swing pause/resume pair, found by the fix above, confirmed real by the user.** Once the stale-zone bug was fixed, the engine surfaced a genuine swing-low pause at **2026-05-06 13:45** (SELL_ONLY -> NONE) and swing-high resume at **2026-05-14 18:00** (NONE -> SELL_ONLY) that the original hand walkthrough never caught -- both land inside what was previously treated as one continuous SELL_ONLY window (04-14 -> 05-29). User confirmed explicitly: **"that is real finding and it must be treated similarly... those pauses exist and must be added."** Not dismissed as non-material (unlike the already-accepted 51-minute 16:00-vs-16:51 timing difference from the rule revision) -- this one changes the actual gate count and window boundaries.

**Updated final gate count: 14 -> 16.** `build_manual_gates()` in `full_viewer.py` now returns all 16 gates (docstring renumbered and rewritten to match), splitting the old gate 1 (04-14 17:55 -> 05-29 17:51, one continuous SELL_ONLY) into three: SELL_ONLY 04-14 17:55 -> 05-06 13:45, NONE 05-06 13:45 -> 05-14 18:00, SELL_ONLY 05-14 18:00 -> 05-29 17:51. Every other gate's timestamp is unchanged from the 14-gate chain, just renumbered (old gate N is now gate N+2 from gate 2 onward).

**Verified against the regenerated output, not just asserted:** `weekly_control_events.csv` now logs all 16 transitions automatically (`SWING_PAUSE`/`SWING_RESUME`/`ZONE_DEATH`/`CAMPAIGN_START_COUNTERTREND`/`OPPOSING_ENCOUNTER`/`OPPOSING_GAINS_CONTROL`), at exact minute-level timestamps that match the hand-verified chain precisely, including zone 9's death now correctly landing at 08-03 00:00 (was silently drifting to the following week before bug 2 was fixed) and zone 8's death at 08-24 00:00. Checked the newly-discovered NONE window directly against `h4_ob_ledger.csv`: 11 H4 OB impacts fall inside 2026-05-06 13:45 -> 05-14 18:00, and every one of them now correctly shows `control_at_impact=NONE, authorized=False` (previously they were wrongly authorized under the old, too-generous single continuous SELL_ONLY gate). Full pipeline regenerated cleanly: 1130 4H bars, 359 H4 OBs computed, 24 shown in the `--manual-gates` window (down from 26, since 11 previously-authorized OBs in the new NONE window are now correctly excluded), 29 5m BSO rows (down from 31).

**`log()` in `weekly_control_engine.py` gained an `at_override` parameter** so events fire at their real underlying minute-level timestamp (swing confirm time, zone impact time, or week-close time) instead of always defaulting to the containing week's start -- needed to get exact times worth comparing against the hand-verified chart timestamps, not just week-level granularity.

**Not yet done:** gates 15-16's (old 13-14's) and the two new gates' (2-3's) H4/5m boxes and entries have not been walked candle-by-candle against the real chart the way the original 04-14 to 08-04 window was -- same outstanding item as before, just renumbered. The phantom weekly Sunday-reopen candle issue (OB #249 worked example) is still open, unrelated to this port. The ~3-pip feed discrepancy is still unverified.

## Session update — 2026-09-18 (gates 15-16 chart-walked candle-by-candle; full entry history reviewed; 2026 dataset now fully closed out)

**Gates 15-16 walked H4-candle-by-candle, Riyadh time, straight from the OHLC/ledger records** (not yet cross-checked against a user chart screenshot this round -- flag if it doesn't match on review):

- **Gate 15, SELL_ONLY (zone 8), 2026-08-19 15:49 -> 2026-08-24 00:00**: zone 8 touched intraday within the 08-19 12:00 H4 candle (O1.15977 H1.16432 L1.15961 C1.16352), confirmed at the 1-minute level at 15:49. H4 OB #324 (SELL IFOB, box 1.16726-1.16982) originates at the 08-20 08:00 candle, triggers on the 08-20 16:00 candle's break, becomes eligible/impacted at 08-21 01:01. Two entry attempts, both stopped out: #22 (2026-08-21 02:34, -1.00R) and #23 (2026-08-21 03:41, -1.00R). Gate closes when the containing Weekly candle (week 08-17->08-24: O1.15620 H1.17112 L1.15602 C1.16718) closes 1.16718, well above zone 8's zt (1.16522, 196-pip margin) -- a full, non-borderline body breach. That Weekly close is sourced in part from the 08-23 20:00 Sunday-reopen H4 candle, the same still-flagged phantom-candle case (OB #249) from earlier -- noted here as a second concrete instance, but the breach margin is large enough that zone 8's death call holds regardless of how that issue eventually resolves.
- **Gate 16, NONE, 2026-08-24 00:00 -> 2026-09-11 22:05 (end of data)**: three H4 OBs (#325, #327, #328) form and resolve that same morning, all correctly blocked (`control_at_impact=NONE, authorized=False`). No Weekly zone reacts again for the rest of the CSV. No further trades. This is the terminal state of the entire 2026 dataset.

**Full entry history reviewed, all 23 entries, start to finish** (zone 3's original impact through end of data), all SELL (matches SELL_ONLY control throughout the whole dataset -- BOTH's brief BUY side, zone 5, never itself produced an entry):

- 8 wins (TP, +3.00R each: entries 1, 4, 6, 11, 13, 18, 19, 21), 15 losses (SL, -1.00R each).
- **Net: +9.00R over the full dataset** (04-14 zone 3 impact through 09-11 22:05 end of data).
- Several H4 OBs needed multiple entry attempts before resolving (OB 178 x2, OB 190 x2, OB 212 x3, OB 324 x2) -- same retry-after-SL pattern throughout, not unique to the final gate.

**2026 dataset now fully closed out.** Every control gate from zone 3's first impact to the last 1-minute bar in the CSV (2026-09-11 22:05) is verified: 16 gates, all reproduced automatically by `weekly_control_engine.py`, all cross-checked against `h4_ob_ledger.csv`/`five_bso_ledger.csv`, and gates 1-16 all walked (11-14 in an earlier entry, 15-16 in this one; 1-10 in the original 2026-09-17 sweep). Final state at end of file: **control = NONE**, zone 8 (the last active Weekly zone) dead, no open or pending H4 OB carrying forward. Net result across the whole file: +9.00R, 23 entries.

**How to close out this OB/dataset and hand off cleanly, for whoever (or whichever future session) picks this up next:**
1. **Nothing needs to be "closed" in code** -- there is no open position, no armed zone, no pending OB at the end of the CSV. The engine's own state at the last processed minute already IS the closed/terminal state (NONE, no controlling zone). Nothing to flatten or cancel.
