...
namespace cAlgo.Robots.ICT_S1
{
    ...
    public class VisualizationManager
    {
        ...
        public void UpdatePoiTerminal(S1PoiSnapshot s)
        {
            if (_mode == VisualizationMode.Off) return;
            ...
            var color = ColorForLifecycle(s.LifecycleState, s.Direction);
            var t = s.RetirementTime ?? DateTime.UtcNow;
            var price = s.RelevantReactionSwingPrice ?? (s.Direction == Direction.Buy ? s.Zt : s.Zb);
            _chart.DrawText(name, s.LifecycleState == S1PoiLifecycleState.Retired ? "RETIRED" : "INVALID", t, price, color);
            Track(name);
        }

        // --- H4 protected swing ---
        public void DrawProtectedSwing(H4Setup setup)
        {
            if (_mode == VisualizationMode.Off) return;
            var name = NextName("PROT_" + setup.H4SetupId);
            _chart.DrawTrendLine(name, setup.ProtectedSwingTime, setup.ProtectedSwingPrice, DateTime.UtcNow, setup.ProtectedSwingPrice, Color.Purple, 1, LineStyle.Dots);
            Track(name);
        }

        // --- M5 pending order / SL / TP ---
        public void DrawAttemptOrder(M5Attempt attempt)
        {
            if (_mode == VisualizationMode.Off) return;
            ...
            _chart.DrawTrendLine(baseName + "_entry", now.AddHours(-1), attempt.RequestedEntryPrice, now.AddHours(1), attempt.RequestedEntryPrice, Color.Blue, 1, LineStyle.Solid);
            _chart.DrawTrendLine(baseName + "_sl", now.AddHours(-1), attempt.SLPrice, now.AddHours(1), attempt.SLPrice, Color.Red, 1, LineStyle.Dots);
            _chart.DrawTrendLine(baseName + "_tp", now.AddHours(-1), attempt.TPPrice, now.AddHours(1), attempt.TPPrice, Color.Green, 1, LineStyle.Dots);
            Track(baseName + "_entry");
            Track(baseName + "_sl");
            Track(baseName + "_tp");
        }

        public void DrawAttemptClosed(M5Attempt attempt)
        {
            if (_mode == VisualizationMode.Off) return;
            ...
            var color = attempt.ExitReason == ExitReason.TakeProfit ? Color.Green
                      ...
                      : Color.Orange;
            var t = attempt.ExitTime ?? DateTime.UtcNow;
